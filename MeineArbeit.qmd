...
if (file.exists(file_sum)) {
  ...
  missing_summary <- bind_rows(missing_rr, missing_or2) %>% relocate(datensatz)
...
if (file.exists(file_hop)) {
  ...
  interpretation <- dplyr::case_when(
    ...
    H_mean < 0.25 ~ "regelmäßige Struktur",
...
if (file.exists(cust_file)) {
  ...
  # Snapshot für Recency
  snapshot <- max(as.POSIXct(purch$timestamp))
  ...
  cust <- orders %>%
    ...
    summarise(
      ...
      last_purchase      = max(snapshot),
      .groups="drop"
    ) %>%
    mutate(recency_days = as.numeric(difftime(snapshot, last_purchase, units="days"))) %>%
    select(-last_purchase)

  # Sortimentsdiversität
  cat_div <- purch %>%
    group_by(customer_id) %>%
    summarise(category_diversity = n_distinct(category), .groups="drop")
  ...
  cust <- cust %>%
    left_join(cat_div, by="customer_id") %>%
    left_join(ret_feat, by="customer_id") %>%
    mutate(across(c(category_diversity, returns_abs, returns_cnt), ~replace_na(.,0)))

  saveRDS(cust, cust_file)
}
...
km_file = "cache/cluster_kmeans.rds"

if os.path.exists(km_file):
    import rpy2.robjects as ro
    km_res = ro.r['readRDS'](km_file)
else:
    cust = r.cust.to_pandas()
    features = [
    ...
    import rpy2.robjects as ro
    ro.r['saveRDS'](ro.conversion.py2rpy(km_res), km_file)
...
hdb_file = "cache/cluster_hdbscan.rds"

if os.path.exists(hdb_file):
    import rpy2.robjects as ro
    hdb_res = ro.r['readRDS'](hdb_file)
else:
    cust = r.cust.to_pandas()
    features = [
        "recency_days","frequency","monetary","aov",
    ...
    import rpy2.robjects as ro
    ro.r['saveRDS'](ro.conversion.py2rpy(hdb_res), hdb_file)
...
if os.path.exists(metrics_file):
    import rpy2.robjects as ro
    metrics = ro.r['readRDS'](metrics_file)
else:
    cust = r.cust.to_pandas()
    features = [
        "recency_days","frequency","monetary","aov",
    ...
    sil = silhouette_score(X, lab_km) if len(set(lab_km))>1 else np.nan
    db  = davies_bouldin_score(X, lab_km) if len(set(lab_km))>1 else np.nan
    ...
    mask = lab_h >= 0
    dbcv = validity_index(X[mask], lab_h[mask]) if mask.sum()>1 and len(set(lab_h[mask]))>1 else np.nan

    metrics = pd.DataFrame([{
        "silhouette_kmeans": sil,
        "davies_bouldin_kmeans": db,
        "dbcv_hdbscan": dbcv,
        "n_customers": X.shape[0]
    }])

    import rpy2.robjects as ro
    ro.r['saveRDS'](ro.conversion.py2rpy(metrics), metrics_file)
