...
Dieser Anhang ergänzt die im Haupttext dargestellten Ergebnisse um detaillierte Tabellen, Visualisierungen und narrative Beschreibungen der identifizierten Segmente. Während Anhang C die technische Umsetzung der Aufbereitung und Modellierung dokumentiert, liegt der Schwerpunkt hier auf der Darstellung, Interpretation und Kommunikation der Ergebnisse.  

\medskip

**Inhalt dieses Anhangs** \smallskip  

1. [Clusterprofile (Kennzahlen je Segment)](#sec-anhangD.1)  
2. [Visualisierungen der Clusterlösungen](#sec-anhangD.2)  
3. [Personas der Kundensegmente](#sec-anhangD.3)  
...
prof_km <- cust_km %>%
  ...
  summarise(across(c(recency_days, frequency, monetary, aov,
...
Zur Veranschaulichung der Cluster wurden Boxplots und Radar-Charts erstellt. Sie ermöglichen einen intuitiven Vergleich der Segmente entlang zentraler Dimensionen.

```{r D.2-visualisierungen_der_clusterloesungen}
...
# Boxplots für ausgewählte Variablen
cust_km %>%
  pivot_longer(cols=c(monetary, recency_days, frequency, basket_items_mean, category_diversity),
               names_to="variable", values_to="value") %>%
  ggplot(aes(x=factor(cluster_kmeans), y=value, fill=factor(cluster_kmeans))) +
  geom_boxplot(outlier.alpha=0.2) +
  facet_wrap(~variable, scales="free_y") +
  labs(title="K-Means Clusterprofile", x="Cluster", y="Wert", fill="Cluster") +
  theme_minimal()
```

\noindent Radar-Charts können zusätzlich erstellt werden, um die typischen Muster pro Cluster auf einen Blick zu erfassen. (Optional, falls du das noch aufnehmen möchtest.)
...
# Skalen für fmsb: oberste + unterste Zeile = Max/Min
radar_df <- rbind(
  apply(df[,-1], 2, max, na.rm=TRUE),
  apply(df[,-1], 2, min, na.rm=TRUE),
  df[,-1]
)
rownames(radar_df) <- c("max","min", paste0("Cluster ", df$cluster_kmeans))
...
radarchart(radar_df,
           ...
           cglty=1,
           axislabcol="grey20",
           caxislabels=seq(0, max(radar_df), length.out=5),
           cglwd=0.8,
           vlcex=0.9,
           title="Radar-Chart der K-Means-Clusterprofile")
...
Die Clusterprofile wurden in narrative Personas übersetzt. Personas helfen, abstrakte Kennzahlen in greifbare Beschreibungen von Kundentypen zu überführen.
  ...
  * Cluster 1 – Loyaler Stammkunde
    Häufige Einkäufe, mittlerer Warenkorbwert, hohe Bindung. Geeignet für Treueprogramme.
  * Cluster 2 – Premium-Gelegenheitskäufer
    Wenige Einkäufe, hoher Umsatz, Premium-orientiert. Ansprache über exklusive Angebote.
  * Cluster 3 – Abwanderungsgefährdet
    Niedrige Frequenz und Umsätze, lange Recency. Reaktivierungskampagnen notwendig.
  * Cluster 4 – Schnäppchenjäger
    Sehr hohe Frequenz, kleine Warenkörbe. Zielgruppe für Rabattaktionen und Promotions.
  * Cluster 5 – Unregelmäßige Käufer (HDBSCAN, teils Rauschen)
    Saisonale Muster, schwer planbares Verhalten. Geeignet für Event-bezogene Kampagnen.
