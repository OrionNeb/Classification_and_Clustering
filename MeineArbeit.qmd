...
Die Qualitätsbewertung dichtebasierter Lösungen erfolgt mit **DBCV** (Density-Based Clustering Validation), das die „within- and between-cluster density connectedness“ misst und den Einfluss von Rauschen sachgerecht einbezieht (@moulaviDensityBasedClusteringValidation2014, S. 839f).
...
- **Evaluation**: Je Verfahren passende Indizes: **Silhouette** und **Davies–Bouldin** für K-Means sowie **DBCV** für HDBSCAN; Stabilität via Bootstrapping/ARI, Clustertendenz vorab via **Hopkins** (vgl. \@sec-validierung und \@sec-evaluation; @laiSilhouetteCoefficientbasedWeighting2025; @moulaviDensityBasedClusteringValidation2014).
...
Das Recency-Merkmal wird nicht im Preprocessing berechnet, sondern erst beim Feature Engineering auf Kundenebene in Abschnitt [C.7](#sec-anhangC7).
...
py$or2_returns <- or2_time %>%
  ...
  select(transactionid, customer_id, timestamp = timestamp_iso,
         revenue_signed, revenue_w)   # <- beide Werte mitgeben
...
if (file.exists(cust_file)) {
  ...
  group_by(customer_id) %>%
  summarise(
    returns_abs   = sum(abs(revenue_w), na.rm = TRUE),      # absoluter Wert der Rückgaben
    returns_cnt   = n_distinct(transactionid),              # Anzahl Rückgaben
    returns_net   = sum(revenue_signed, na.rm = TRUE),      # negative Summen explizit
    .groups="drop"
  ) %>%
  mutate(
    returns_ratio = if_else(returns_abs > 0, returns_abs / abs(returns_net), 0)
  )

  cust <- cust %>%
  left_join(dist_tab, by="customer_id") %>%
  left_join(ret_feat, by="customer_id") %>%
  mutate(across(c(category_diversity, hhi_rev, entropy_rev,
                  returns_abs, returns_cnt, returns_net, returns_ratio),
                ~replace_na(.,0))) %>%
  mutate(
...
write.csv(cust, "cache/customer_features.csv", row.names = FALSE)
}
...
# cust direkt aus CSV laden (nicht aus R!)
cust = pd.read_csv("cache/customer_features.csv")
...
if (file.exists(metrics_file)) {
  ...
  cust <- read.csv("cache/customer_features.csv")
  features <- c(
    "recency_days","frequency","monetary","aov",
    "basket_items_mean","basket_items_p95","category_diversity",
    "hhi_rev","entropy_rev","returns_abs","returns_cnt",
...
::: {#sec-anhangD}
# Anhang D: Ergebnisse und Visualisierungen {.unnumbered}
:::
\markboth{Anhang D: Ergebnisse und Visualisierungen}{}

Dieser Anhang ergänzt die im Haupttext dargestellten Ergebnisse um detaillierte Tabellen, Visualisierungen und narrative Beschreibungen der identifizierten Segmente. Während [Anhang C](#sec-anhangC) die technische Umsetzung der Aufbereitung und Modellierung dokumentiert, liegt der Schwerpunkt hier auf der Darstellung, Interpretation und Kommunikation der Ergebnisse.  

\medskip

\myheading{Inhalt dieses Anhangs} \smallskip  

1. [Clusterprofile (Kennzahlen je Segment) (D.1)](#sec-anhangD.1)
2. [Visualisierungen der Clusterlösungen (D.2)](#sec-anhangD.2)
3. [Personas der Kundensegmente (D.3)](#sec-anhangD.3)

\clearpage

::: {#sec-anhangD.1}
## D.1: Clusterprofile {.unnumbered}
:::

Die Clusterlösungen aus [C.7](#sec-anhangC.7) wurden aggregiert, um zentrale Kennzahlen je Segment zu berechnen. Diese **Clusterprofile** zeigen Mittelwerte, Mediane und Streuungen wichtiger Merkmale (Recency, Frequency, Monetary, Warenkorb- und Sortimentskennzahlen). Sie bilden die Grundlage für die spätere Interpretation und die Ableitung von Personas.  

```{r D.1-clusterprofile}

#| eval: true
#| echo: true
#| message: false
#| warning: false
#| dependson: ["C.7-validierung"]
...
# Basisdaten laden
cust <- read.csv("cache/customer_features.csv")
km   <- read_csv("cache/cluster_kmeans.csv", show_col_types = FALSE)
hdb  <- read_csv("cache/cluster_hdbscan.csv", show_col_types = FALSE)

# Clusterlabels anhängen
cust_km  <- cust %>% left_join(km,  by="customer_id")
cust_hdb <- cust %>% left_join(hdb, by="customer_id")

# Variablen für Profilbildung
features <- c("recency_days","frequency","monetary","aov",
              "basket_items_mean","basket_items_p95",
              "category_diversity","hhi_rev","entropy_rev",
              "returns_abs","returns_cnt")

# Profile berechnen: K-Means
prof_km <- cust_km %>%
...
# Profile berechnen: HDBSCAN (nur Cluster >= 0)
prof_hdb <- cust_hdb %>%
...
# speichern
saveRDS(list(kmeans=prof_km, hdbscan=prof_hdb), "cache/cluster_profiles.rds", compress="xz")

list(KMeans=prof_km, HDBSCAN=prof_hdb)
```

\noindent Die Tabellen zeigen, welche Unterschiede sich zwischen den Segmenten ergeben – z. B. besonders hohe Umsätze (Monetary), kurze Kaufintervalle (Recency) oder hohe Sortimentsdiversität.

::: {#sec-anhangD.2}
## D.2: Visualisierungen der Clusterlösungen {.unnumbered}
:::

Zur Veranschaulichung der Cluster wurden zwei komplementäre Visualisierungsformen gewählt: \smallskip

(1) Boxplots, die die Streuung der einzelnen Variablen je Cluster in absoluten Werten zeigen,  
(2) Radar-Charts, die die Clusterprofile auf eine vergleichbare 0–1 Skala normieren.  

So können sowohl absolute Unterschiede als auch relative Muster erkannt werden.  


```{r D.2-boxplots, fig.width=12, fig.height=6}

#| eval: true
#| echo: true
#| message: false
#| warning: false
#| dependson: ["D.1-clusterprofile"]

library(ggplot2)
library(dplyr)
library(readr)

# Cluster-Ergebnisse laden
km_res <- read_csv("cache/cluster_kmeans.csv", show_col_types = FALSE)
cust   <- read.csv("cache/customer_features.csv")

df <- cust %>%
  inner_join(km_res, by = "customer_id") %>%
  select(cluster_kmeans, recency_days, frequency, monetary,
         basket_items_mean, category_diversity) %>%
  tidyr::pivot_longer(-cluster_kmeans, names_to = "Merkmal", values_to = "Wert")

# Labels schöner formatieren
df$Merkmal <- recode(df$Merkmal,
  "basket_items_mean"  = "Ø Warenkorbgröße",
  "category_diversity" = "Sortiments-\nDiversität",
  "frequency"          = "Kauffrequenz",
  "monetary"           = "Monetary (Umsatz)",
  "recency_days"       = "Recency (Tage)"
)

# Legenden-Labels (inkl. Personas)
cluster_labels <- c(
  "0 – Loyale Stammkunden",
  "1 – Premium-Gelegenheitskäufer",
  "2 – Abwanderungsgefährdet",
  "3 – Schnäppchenjäger",
  "4 – Unregelmäßige Käufer"
)

ggplot(df, aes(x=factor(cluster_kmeans), y=Wert, fill=factor(cluster_kmeans))) +
  geom_boxplot(outlier.size=0.8, alpha=0.7) +
  facet_wrap(~Merkmal, scales="free", labeller = label_wrap_gen(width = 15)) +
  scale_fill_brewer(palette="Set2", name="Cluster", labels=cluster_labels) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE)) +   # << Legende zweizeilig
  theme_minimal(base_size=13) +
  theme(
    legend.position="bottom",
    legend.title = element_text(face="bold"),
    legend.text = element_text(size=10),
    strip.text = element_text(size=12, face="bold", lineheight = 1.1),
    axis.text.x = element_text(size=10),
    axis.title.y = element_text(size=11)
  ) +
  labs(
    title="K-Means Clusterprofile (Boxplots)",
    x="Cluster", y="Wert"
  )
```

Radar-Charts können zusätzlich erstellt werden, um die typischen Muster pro Cluster auf einen Blick zu erfassen. (Optional, falls du das noch aufnehmen möchtest.)

Die Radar-Darstellung zeigt die Clusterprofile normiert (0–1), damit die Variablen trotz unterschiedlicher Größenordnungen vergleichbar sind. Um Überlagerungen zu vermeiden, wird jedes Cluster in einem eigenen Radar dargestellt.
...
# Clusterprofile laden
profiles <- readRDS("cache/cluster_profiles.rds")$kmeans

# Variablen auswählen
vars <- c("recency_days_mean","frequency_mean","monetary_mean",
          "basket_items_mean_mean","category_diversity_mean")

df <- profiles %>%
  select(cluster_kmeans, all_of(vars)) %>%
  rename_with(~sub("_mean","",.), all_of(vars)) %>%
  as.data.frame()

# Normalisieren (0–1)
df_norm <- df
df_norm[,-1] <- lapply(df[,-1], function(x) rescale(x, to=c(0,1)))

# Achsenlabels schöner
colnames(df_norm)[-1] <- c("Recency (Tage)","Kauffrequenz","Monetary (norm.)",
                           "Ø Warenkorbgröße","Sortiments-Diversität")

# Farben (Set2, wie Boxplots)
cluster_colors <- brewer.pal(5,"Set2")
names(cluster_colors) <- as.character(0:4)

# Persona-Namen für Legende
persona_labels <- c(
  "0" = "Cluster 0 – Loyaler Stammkunde",
  "1" = "Cluster 1 – Premium-Gelegenheitskäufer",
  "2" = "Cluster 2 – Abwanderungsgefährdet",
  "3" = "Cluster 3 – Schnäppchenjäger",
  "4" = "Cluster 4 – Unregelmäßige Käufer"
)

# Layout 2x3 (5 Plots + 1 Feld frei)
par(mfrow=c(2,3), mar=c(2,2,3,2))
for (i in seq_len(nrow(df_norm))) {
  radar_df <- rbind(
    rep(1, ncol(df_norm)-1),
    rep(0, ncol(df_norm)-1),
    df_norm[i,-1]
  )
  rownames(radar_df) <- c("max","min", paste0("Cluster ", df_norm$cluster_kmeans[i]))
  
  radarchart(radar_df,
             axistype=1,
             pcol=cluster_colors[as.character(df_norm$cluster_kmeans[i])],
             pfcol=adjustcolor(cluster_colors[as.character(df_norm$cluster_kmeans[i])], alpha.f=0.3),
             plwd=2,
             plty=1,
             cglcol="grey",
             cglty=1,
             ...
             cglwd=0.8,
             vlcex=0.9,
             title=paste("Cluster", df_norm$cluster_kmeans[i]))  # nur Cluster-ID
}

# Gemeinsame Legende unten (zweizeilig, wie Boxplots)
par(mfrow=c(1,1))
plot.new()
legend("center",
       legend=persona_labels,
       col=cluster_colors,
       lty=1, lwd=2, bty="n",
       ncol=2, cex=0.9)
...
::: {#sec-anhangD.3}
## D.3: Personas der Kundensegmente {.unnumbered}
:::

Die Clusterprofile wurden in narrative Personas übersetzt. Personas helfen, abstrakte Kennzahlen in greifbare Beschreibungen von Kundentypen zu überführen.  
Zur besseren Wiedererkennbarkeit werden die Cluster in den gleichen Farben wie in den Visualisierungen (vgl. [D.2](#sec-anhangD.2)) dargestellt.
...
personas <- tibble::tibble(
  Cluster = c("Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5"),
  Persona = c(
    "Loyaler Stammkunde: Häufige Einkäufe, mittlerer Warenkorbwert, hohe Bindung. Geeignet für Treueprogramme.",
    "Premium-Gelegenheitskäufer: Wenige Einkäufe, hoher Umsatz, Premium-orientiert. Ansprache über exklusive Angebote.",
    "Abwanderungsgefährdet: Niedrige Frequenz und Umsätze, lange Recency. Reaktivierungskampagnen notwendig.",
    "Schnäppchenjäger: Sehr hohe Frequenz, kleine Warenkörbe. Zielgruppe für Rabattaktionen und Promotions.",
    "Unregelmäßige Käufer (HDBSCAN, teils Rauschen): Saisonale Muster, schwer planbares Verhalten. Geeignet für Event-bezogene Kampagnen."
  )
)

if (knitr::is_latex_output()) {
  # Fallback für PDF: einfache Tabelle ohne Farben
  knitr::kable(personas, format="latex", booktabs=TRUE)
} else {
  # Farben nur in HTML
  cluster_colors <- c("Cluster 1"="#FF0000","Cluster 2"="#0000FF",
                      "Cluster 3"="#008000","Cluster 4"="#FFA500","Cluster 5"="#800080")
  personas %>%
    gt() %>%
    data_color(
      columns = vars(Cluster),
      colors = scales::col_factor(palette = unname(cluster_colors),
...
\noindent Diese Personas schlagen die Brücke von der Modellierung zur praktischen Nutzung im Marketing und CRM. Sie machen die Ergebnisse verständlich und handlungsleitend.
