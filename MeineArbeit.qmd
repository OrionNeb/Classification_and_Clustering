...
```{r c3-bereinigung, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE, cache=TRUE}
# benötigte Pakete
library(dplyr)
library(stringr)
...
# Dateien: 
#   cache/rr_time.rds  = normalisierte RetailRocket-Daten
#   cache/or2_time.rds = normalisierte Online-Retail-II-Daten
...
to_iso_utc <- function(x) {
  format(with_tz(as.POSIXct(x, tz = "UTC"), "UTC"), "%Y-%m-%dT%H:%M:%SZ")
}
...
list(
  ...
  or2_time_head = head(select(or2_time, customer_id, item_id, timestamp, timestamp_iso, event_type,
                              quantity_w, price_w, revenue_w), 3)
)
...
# Datei: cache/zusammenfassung.rds = Dimensionen, Missing, Duplikate, Verteilungen, Ereignisse, Abdeckung

#| eval: true
#| echo: true
#| message: false
#| warning: false
#| dependson: ["c4-zeit-normalisierung"]
...
# Guards
if (!exists("rr_time") || !exists("or2_time")) {
  stop("C.5 benötigt rr_time und or2_time aus C.4.")
}
...
if (file.exists(file_sum)) {
  ...
  # 2) Vollständigkeit zentraler Felder
  missing_rr  <- rr_time  %>%
    summarise(across(c(customer_id, item_id, timestamp, event_type), ~ sum(is.na(.)))) %>%
    pivot_longer(everything(), names_to = "feld", values_to = "missing") %>%
    mutate(datensatz = "RetailRocket")

  missing_or2 <- or2_time %>%
    summarise(across(c(customer_id, item_id, timestamp, event_type, quantity_w, price_w, revenue_w), ~ sum(is.na(.)))) %>%
    pivot_longer(everything(), names_to = "feld", values_to = "missing") %>%
    mutate(datensatz = "Online Retail II")

  missing_summary <- bind_rows(missing_rr, missing_or2) %>%
    relocate(datensatz)

  # 3) Duplikatprüfung auf technischem Schlüssel
  dup_rr <- rr_time %>%
    mutate(key = paste(customer_id, item_id, timestamp, event_type, sep = "|")) %>%
    summarise(dups = sum(duplicated(key)), share = round(mean(duplicated(key)), 6)) %>%
    mutate(datensatz = "RetailRocket")
  ...
  duplicate_summary <- bind_rows(dup_rr, dup_or2) %>% relocate(datensatz)

  # 4) Verteilungen kaufrelevanter Variablen (nur OR2)
  ...
  dist_or2 <- or2_time %>%
    ...
    summarise(
      n         = n(),
      missing   = sum(is.na(wert)),
      mean      = mean(wert, na.rm = TRUE),
      sd        = sd(wert, na.rm = TRUE),
      q0        = quantile(wert, probs = quantiles[1], na.rm = TRUE),
      q01       = quantile(wert, probs = quantiles[2], na.rm = TRUE),
      q05       = quantile(wert, probs = quantiles[3], na.rm = TRUE),
      q25       = quantile(wert, probs = quantiles[4], na.rm = TRUE),
      q50       = quantile(wert, probs = quantiles[5], na.rm = TRUE),
      q75       = quantile(wert, probs = quantiles[6], na.rm = TRUE),
      q95       = quantile(wert, probs = quantiles[7], na.rm = TRUE),
      q99       = quantile(wert, probs = quantiles[8], na.rm = TRUE),
      q100      = quantile(wert, probs = quantiles[9], na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(across(where(is.numeric), ~ round(., 4)))

  # 5) Ereignisverteilung
  evt_rr  <- rr_time  %>% count(event_type, name = "n") %>% mutate(datensatz = "RetailRocket") %>% relocate(datensatz)
  evt_or2 <- or2_time %>% count(event_type, name = "n") %>% mutate(datensatz = "Online Retail II") %>% relocate(datensatz)
  event_summary <- bind_rows(evt_rr, evt_or2)

  # 6) Zeitliche Abdeckung nach Jahr-Monat
  cover_rr <- rr_time %>%
    mutate(year_month = format(as.Date(timestamp), "%Y-%m")) %>%
    count(year_month, name = "n") %>%
    mutate(datensatz = "RetailRocket") %>%
    select(datensatz, everything())
  ...
  coverage_summary <- bind_rows(cover_rr, cover_or2) %>% arrange(datensatz, year_month)

  # 7) Ausgabe bündeln
  res <- list(
    dimensionen         = dim_summary,
    fehlende_werte      = missing_summary,
    duplikate           = duplicate_summary,
    verteilungen_or2    = dist_or2,
    ereignisse          = event_summary,
    zeitliche_abdeckung = coverage_summary,
    beispiel_rr         = head(select(rr_time, customer_id, item_id, timestamp, event_type), 5),
    beispiel_or2        = head(select(or2_time, customer_id, item_id, timestamp, event_type,
                                      quantity_w, price_w, revenue_w), 5)
  )

  saveRDS(res, file_sum)
}
...
# Datei: cache/hopkins.rds = Ergebnis Hopkins-Statistik (inkl. CI und Einordnung)
...
# Guard
if (!exists("or2_time")) {
  stop("C.6 benötigt 'or2_time' aus C.4.")
}

# Optionaler Schnellmodus aus C.4; falls nicht gesetzt, hier definieren
...
if (file.exists(file_hop)) {
  ...
  # Parameter
  set.seed(42)
  HOP_NROWS <- if (FAST) 1500L else 5000L   # maximale Stichprobengröße
  HOP_R     <- if (FAST) 10L    else 25L    # Wiederholungen für CI
  HOP_N     <- 300L                         # n für clustertend::hopkins
  ...
  # Helper robust gegenüber Rückgabeformaten unterschiedlicher clustertend Versionen
  ...
  # Mehrfachschätzung und Intervall
  ...
  interpretation <- dplyr::case_when(
    H_mean > 0.75 ~ "deutliche Clustertendenz",
    H_mean < 0.25 ~ "regelmäßige anti clusternde Struktur",
...
# Datei: cache/hopkins_grid.rds = Sensitivitätsanalyse Hopkins-Statistik (verschiedene n)

#| eval: !FAST
#| echo: true
#| message: false
#| warning: false
#| dependson: ["c6-hopkins"]

dir.create("cache", showWarnings = FALSE)
file_grid <- "cache/hopkins_grid.rds"

if (file.exists(file_grid)) {
  grid_tbl <- readRDS(file_grid)
} else {
  set.seed(42)
  n_grid <- c(50L, 100L, 200L, min(300L, nrow(X) - 1L))

  grid_tbl <- lapply(n_grid, function(n) {
    vals <- replicate(25L, get_H(X, n))
    tibble(
      hopkins_n = n,
      mean      = mean(vals),
      sd        = sd(vals),
      ci_low    = mean(vals) - 1.96 * sd(vals) / sqrt(length(vals)),
      ci_up     = mean(vals) + 1.96 * sd(vals) / sqrt(length(vals))
    )
  }) %>% dplyr::bind_rows() %>%
    mutate(across(where(is.numeric), ~ round(., 3)))

  saveRDS(grid_tbl, file_grid)
}
...
::: {#sec-anhangC.7}
## C.7 Modellierungsschritte mit Python: K-Means und HDBSCAN {.unnumbered}
:::
...
Im Folgenden wird exemplarisch ein Startwert von fünf Clustern gewählt. Die eigentliche Validierung, ob diese Zahl angemessen ist, erfolgt nicht hier, sondern systematisch in [Kapitel 4: Evaluation und Ergebnisse](#sec-evaluation) anhand von Silhouette, Davies-Bouldin und Stabilitätsmaßen.

```{python C.7-kmeans}



# Datei: cache/cluster_kmeans.parquet = KMeans-Ergebnisse (k=5)
...
out_file = "cache/cluster_kmeans.parquet"

if os.path.exists(out_file):
    cluster_kmeans_df = pd.read_parquet(out_file)
else:
    np.random.seed(42)

    # Daten aus R (C.4 übergibt timestamp als ISO-String)
    df = or2_time.copy()
    ...
    # Baseline k=5; Bewertung und Optimierung erfolgen in 5.2
    ...
    # Für reticulate nach R exportieren
    cluster_kmeans_df = out

    # Datei speichern
    cluster_kmeans_df.to_parquet(out_file, index=False)

cluster_kmeans_df.head(10)
...
\noindent Das Ergebnis ist eine Zuordnung jeder Beobachtung zu einem Cluster. Die Details zur Beurteilung der Clusterqualität und -anzahl erfolgen später in [Kapitel 4](#sec-evaluation).

\myheading{HDBSCAN}

HDBSCAN bildet als dichtebasiertes Verfahren ungleichförmige Strukturen und Rauschen ab. Die Verfahrenseigenschaften sind in Abschnitt 4.2 beschrieben. Die Bewertung der resultierenden Struktur erfolgt in Abschnitt 5.2; die inhaltliche Einordnung der Segmente folgt in Abschnitt 5.3.

HDBSCAN ist ein hierarchisch-dichtebasiertes Verfahren, das im Gegensatz zu K-Means keine feste Clusteranzahl vorgibt. Stattdessen werden Regionen hoher Dichte als Cluster erkannt, während dünn besetzte Bereiche als Rauschen ausgewiesen werden. Damit eignet sich HDBSCAN besonders für komplex geformte und unterschiedlich große Cluster.

```{python C.7-hdbscan}


# Datei: cache/cluster_hdbscan.parquet = HDBSCAN-Ergebnisse
...
try:
    ...
    raise SystemExit("HDBSCAN ist nicht installiert. Bitte ausführen: pip install hdbscan")

os.makedirs("cache", exist_ok=True)
out_file = "cache/cluster_hdbscan.parquet"

if os.path.exists(out_file):
    cluster_hdbscan_df = pd.read_parquet(out_file)
else:
    df = or2_time.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)
    ...
    X = StandardScaler(copy=True).fit_transform(X_np)
    ...
    # Datei speichern
    cluster_hdbscan_df.to_parquet(out_file, index=False)

cluster_hdbscan_df.head(10)
...
try:
    ...
    raise SystemExit("Bitte hdbscan installieren: pip install hdbscan")

os.makedirs("cache", exist_ok=True)
out_file = "cache/validation.json"
...
if os.path.exists(out_file):
    ...
    # Silhouette für die K-Means-Baseline k=5
    km = KMeans(n_clusters=5, random_state=42, n_init=10)
    lab_km = km.fit_predict(X)
    sil = float(silhouette_score(X, lab_km)) if len(np.unique(lab_km)) > 1 else None

    # DBCV für HDBSCAN
    hdb = hdbscan.HDBSCAN(min_cluster_size=50, min_samples=5,
                          metric="euclidean", cluster_selection_method="eom",
                          approx_min_span_tree=False, core_dist_n_jobs=1)
    lab_h = hdb.fit_predict(X)

    mask = lab_h >= 0
    dbcv = float(validity_index(X[mask], lab_h[mask], metric="euclidean")) if mask.sum() > 1 and len(np.unique(lab_h[mask])) > 1 else None

    results = {"silhouette_kmeans_k5": sil, "dbcv_hdbscan": dbcv}
...
if (reticulate::py_has_attr(py, "cluster_kmeans_df")) {
  head(py$cluster_kmeans_df, 5)
} else {
  cat("cluster_kmeans_df nicht verfügbar\n")
}

if (reticulate::py_has_attr(py, "cluster_hdbscan_df")) {
...
# Datei: cache/bootstrap_ari.rds = Bootstrap-ARI für KMeans-Stabilität
...
set.seed(42)
dir.create("cache", showWarnings = FALSE)
ari_file <- "cache/bootstrap_ari.rds"

if (file.exists(ari_file)) {
  ari_res <- readRDS(ari_file)
} else {
  # Optionaler Schnellmodus aus C.4; falls nicht gesetzt, hier definieren
  if (!exists("FAST")) FAST <- TRUE

  # Daten vorbereiten (optional: Downsampling für große Datenmengen)
  N_MAX <- if (FAST) 100000L else Inf

  dat <- or2_time %>%
    select(quantity_w, price_w, revenue_w) %>%
    filter(if_all(everything(), is.finite(.)))

  if (is.finite(N_MAX) && nrow(dat) > N_MAX) {
    dat <- dplyr::slice_sample(dat, n = N_MAX)
  }

  X <- scale(as.matrix(dat))

  # k aus vorherigem K-Means übernehmen, falls vorhanden; sonst Baseline k = 5
  k <- 5L
  if (reticulate::py_has_attr(py, "cluster_kmeans_df")) {
    k <- dplyr::n_distinct(py$cluster_kmeans_df$cluster_kmeans)
    if (is.na(k) || k < 2L) k <- 5L
  }

  # Bootstrap-Parameter
  B <- if (FAST) 10L else 20L
  aris <- numeric(B)

  # ARI misst die Übereinstimmung zweier unabhängiger Initialisierungen
  # auf identischer Bootstrap-Stichprobe und quantifiziert so die
  # Stabilität der K-Means-Zuordnung (Bewertung in 5.2)
  for (b in seq_len(B)) {
    idx <- sample(seq_len(nrow(X)), replace = TRUE)
    km1 <- stats::kmeans(X[idx, , drop = FALSE], centers = k, nstart = 10)
    km2 <- stats::kmeans(X[idx, , drop = FALSE], centers = k, nstart = 10)
    aris[b] <- mclust::adjustedRandIndex(km1$cluster, km2$cluster)
  }
